...
def calcular_metricas(datos):
    ...
    margen_promedio = (datos['Ingreso_total'].sum() - datos['Costo_total'].sum()) / datos['Ingreso_total'].sum() * 100
    ...
    for producto in datos["Producto"].unique():
        ...
        with st.container(border=True):
            ...
            with col1:
                ...
                precio_prom, margen_prom, unidades, var_precio, var_margen, var_unidades = calcular_metricas(datos_producto)
                
                st.metric(
                    label="Precio Promedio",
                    value=f"${formatear_numero(precio_prom)}",
                    delta=f"{var_precio:+.2f}%"
                )
                
                st.metric(
                    label="Margen Promedio",
                    value=f"{margen_prom:.0f}%",
                    delta=f"{var_margen:+.2f}%"
                )
                
                st.metric(
                    label="Unidades Vendidas",
                    value=formatear_numero(unidades),
                    delta=f"{var_unidades:+.2f}%"
                )
